...
class IMP implements MouseListener {
   ...
   * and then put the RGB back to an int (getPixels)
     */
    private void fun2() {
        Stack<PixelRegion> stack = new Stack();  //create the stack to store pixels
        PixelRegion click = new PixelRegion(colorX, colorY); //inital pixel clicked
        int compColor[] = new int[4];
        compColor = getPixelArray(picture[colorY][colorX]);  //created as initial click color to compare future pixel colors
        int rgbArray[] = new int[4];  //array created to change pixels to black later on
        rgbArray = getPixelArray(picture[colorY][colorX]); //get the rgb values of the pixel
        rgbArray[1] = 0;  //change pixel rgb values to 0 (black)
        rgbArray[2] = 0;
        rgbArray[3] = 0;
        picture[colorY][colorX] = getPixels(rgbArray);  //set pixel at y,x to black
        stack.push(click);  //put the clicked on pixel on the stack
        while (!stack.empty()) { //while the stack is not empty, check pixels
            PixelRegion top = stack.peek();  //set a reference to the PixelRegion on top so we can call methods on it
            switch (top.getCounter()) {
                case 1:
                    if (getPixels(getPixelArray(picture[top.getPixelY() - 1][top.getPixelX() - 1])) == getPixels(compColor)) { //if pixel color at this coord is same as clicked color
                        picture[top.getPixelY() - 1][top.getPixelX() - 1] = getPixels(rgbArray);  //set pixel at coord to black
                        top.count(); //increment the top pixel counter check to next surrounding pixel
                        stack.push(new PixelRegion(top.getPixelX() - 1, top.getPixelY() - 1));  //put the newly checked pixel on the stack
                        break; //return to the while loop
                    } else {
                        top.count(); //check the next pixel since the current one isn't within color range
                        break;
                    }
                case 2:
                    if (getPixels(getPixelArray(picture[top.getPixelY()][top.getPixelX() - 1])) == getPixels(compColor)) {
                        picture[top.getPixelY()][top.getPixelX() - 1] = getPixels(rgbArray);
                        top.count();
                        stack.push(new PixelRegion(top.getPixelX() - 1, top.getPixelY()));
                        break;
                    } else {
                        top.count();
                        break;
                ...
                case 3:
                    if (getPixels(getPixelArray(picture[top.getPixelY() + 1][top.getPixelX() - 1])) == getPixels(compColor)) {
                        picture[top.getPixelY() + 1][top.getPixelX() - 1] = getPixels(rgbArray);
                        top.count();
                        stack.push(new PixelRegion(top.getPixelX() - 1, top.getPixelY() + 1));
                        break;
                    } else {
                        top.count();
                        break;
                ...
                case 4:
                    if (getPixels(getPixelArray(picture[top.getPixelY() + 1][top.getPixelX()])) == getPixels(compColor)) {
                        picture[top.getPixelY() + 1][top.getPixelX()] = getPixels(rgbArray);
                        top.count();
                        stack.push(new PixelRegion(top.getPixelX(), top.getPixelY() + 1));
                        break;
                    } else {
                        top.count();
                        break;
                ...
                default:  //when we get to pixel counter above 8, pop the pixel off the stack
                    stack.pop();
                    break;
            }
        }//*/
        resetPicture();  //redraw picture, once the stack is empty, so we can see the changes
